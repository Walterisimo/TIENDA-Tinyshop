...
import { updateCategory } from '../../../store/products'
import { useReadLocalStorage } from 'usehooks-ts'

const EditCategory = () => {

  const dispatch = useDispatch()
  const { alert } = useSelector(store => store.products)

  const category = useReadLocalStorage('category')

  const [name, setName] = useState(category.name)
  const [order, setOrder] = useState(category.position)
    ...
    <>
      <form className='' onSubmit={handleForm}>
        <div className='p-6 space-y-6'>
          ...
          <div className='pt-2'>
            <label htmlFor='active' className='block mb-2 text-sm font-medium text-gray-900 dark:text-white'>Visibilidad</label>
        ...
        <div className='flex items-center justify-end p-4 border-t border-gray-200 rounded-b dark:border-gray-600'>
          <button type='submit' className='text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded text-sm px-5 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'>Editar categoría</button>
        </div>
      </form>
    </>
  )
