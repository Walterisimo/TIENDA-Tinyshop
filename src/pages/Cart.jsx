...
const Cart = () => {
  ...
  const sendMessage = () => {
    ...
    const recorrerPedidos = cart.map(item => (
      pedido += `*${item.name}*%0ACantidad%3A%20*${item.quantity}*%0APrecio%3A%20%24%20*${item.price.toLocaleString('es-AR')}*%0ASubtotal%3A%20%24%20*${item.subtotal.toLocaleString('es-AR')}*%0A`
    ))
